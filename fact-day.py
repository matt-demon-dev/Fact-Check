...
class FactBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        # Start scheduled task after syncing
...
# Fetch fact (API first, fallback scraping)
def fetch_fact() -> str:
    try:
        res = requests.get("https://uselessfacts.jsph.pl/random.json?language=en", timeout=10)
...
def schedule_task():
    ...
    now = datetime.now(tz)
    run_dt = now.replace(hour=config["hour"], minute=config["minute"], second=0, microsecond=0)
...
async def post_fact_daily():
    channel = bot.get_channel(config["channel_id"]) or await bot.fetch_channel(config["channel_id"])
    fact = fetch_fact()
...
async def on_ready():
    ...
    if not post_fact_daily.is_running() and config["channel_id"]:
        schedule_task()
...
async def slash_setchannel(interaction: discord.Interaction):
    ...
    save_config()
    schedule_task()
    await interaction.response.send_message("✅ This channel is set for daily facts.", ephemeral=True)
...
@app_commands.checks.has_permissions(administrator=True)
async def slash_settime(interaction: discord.Interaction, hhmm: str, timezone: str):
    try:
        ...
        save_config()
        schedule_task()
        await interaction.response.send_message(f"✅ Time updated to {hour:02}:{minute:02} {timezone}", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(
            "❌ Invalid format or timezone. Example: `/settime 15:30 America/New_York`\n"
...
async def slash_status(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"📢 Channel: {ch}\n🕒 {config['hour']:02}:{config['minute']:02']} {config['timezone']}", ephemeral=True)
...
async def slash_timezones(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("✅ Example Timezones:\n" + "\n".join(examples) + "\nFull list: https://en.wikipedia.org/wiki/List_of_tz_database_time_zones", ephemeral=True)
